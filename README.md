...
自动测试覆盖关键控件对象名、排序开关与旧 `manual` 状态映射、下拉箭头、主按钮启用/禁用像素、switch 滑块位置、方形勾选框和 playback/auto-audition 的既有连接。Windows 源码环境视觉验收和 Windows EXE 最终人工验收均已通过。
...
V2.4-A 已完成对当前选中完整片段的手动试听，直接使用源 `base.ogg`，支持播放、暂停、恢复、循环、有效倍速与波形播放头。V2.4-B 已完成默认关闭且不持久化的自动试听：正式编辑提交和用户显式选择完整卡片或时间线片段后，按 200ms 防抖安排播放，快速连续选择只播放最后一个片段。手动播放、输入编辑、拖动、删除、切歌和关闭窗口都会取消 pending。
...
V2.5-A 已建立多难度的纯逻辑基础，但当前正式切片仍固定处理 `2.aff`。已确认的标准映射为 `0.aff` = Past/PST、`1.aff` = Present/PRS、`2.aff` = Future/FTR、`3.aff` = Beyond/BYD、`4.aff` = Eternal/ETR；歌曲目录实际存在哪些标准 AFF，就可发现哪些难度。任意子集都合法，不要求 0/1/2 同时存在，3/4 同时存在也合法。难度选择 UI、多 AFF 正式写盘和多难度 songlist 仍属于 V2.5-B。
