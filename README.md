...
当前 UI 使用统一的语义色彩与控件样式：只读路径以信息展示呈现，文本输入、下拉选择、开关和按钮具有不同的静态外观，禁用操作仍保留可识别轮廓。该改造只调整视觉表达，不改变切片和导出数据结构、播放/循环/自动试听、排序、dirty 或撤销历史语义。

自动测试已覆盖关键控件对象名、排序开关与旧 `manual` 状态映射、下拉箭头、主按钮禁用态，以及 playback/auto-audition 的既有连接。仍需在 Windows 上完成中文字体、缩放和打包 EXE 的最终人工视觉验收。
