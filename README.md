...
V2.4-B 已加入默认关闭的“自动试听”：当前选中片段的有效区间或继承的有效倍速在正式提交后，会以 200ms 防抖自动试听；输入和拖动过程中不会反复重启。V2.4-C 修复了 EXE 验收中发现的显式选择遗漏：开启自动试听后，点击片段卡片或时间线片段都会在刷新试听范围后安排 200ms 防抖；快速连续选择只会播放最后一个片段。手动播放、输入编辑和生命周期操作仍优先取消等待中的自动试听。自动试听仍使用源 `base.ogg`，不会生成临时音频，也不影响导出、dirty 状态或撤销历史。
