...
- 波形与时间线片段编辑：拖拽空白区创建、端点调整、选中片段单击定位播放、整体平移、草稿预览与 Ctrl+单击快速草稿
...
自动测试覆盖关键控件对象名、排序开关与旧 `manual` 状态映射、下拉箭头、主按钮启用/禁用像素、switch 滑块位置、方形勾选框和 playback/auto-audition 的既有连接。Windows 源码环境人工视觉验收已通过；尚未进行 Windows EXE 验收，V2.4-C 的 EXE/OGG 稳定性仍待执行。
...
V2.4-B 已加入默认关闭的“自动试听”：当前选中片段的有效区间或继承的有效倍速在正式提交后，会以 200ms 防抖自动试听；输入和拖动过程中不会反复重启。V2.4-C 修复了 EXE 验收中发现的显式选择遗漏：开启自动试听后，点击片段卡片或时间线片段都会在刷新试听范围后安排 200ms 防抖；快速连续选择只会播放最后一个片段。已选中时间线片段可单击定位并立即试听，或拖动整体平移；未选中片段首次点击只负责选择，端点 handle 保持优先。定位播放不改变片段边界、dirty 或撤销历史；平移保持片段长度、级联组区间一致，并只提交一条“平移片段”历史。循环仍使用完整片段的原始起止边界。手动播放、输入编辑和生命周期操作仍优先取消等待中的自动试听。自动试听仍使用源 `base.ogg`，不会生成临时音频，也不影响导出、dirty 状态或撤销历史。

V2.4-C 仍待 EXE 人工复验：连续切点修改的真实听感、倍速与循环边界、快速连续选择、输入/手动播放取消 pending，以及打包 EXE 的 OGG 自动试听稳定性。
...
- V2.4-C 的 Windows EXE OGG 手动试听与自动试听稳定性仍待人工复验；V2.4 尚未标记为完整完成。
