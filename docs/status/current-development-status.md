...
V2.4-C 在 Windows EXE 验收中发现显式选择片段没有安排自动试听：卡片和时间线选择只刷新试听范围。修复后，开启自动试听时，显式选择会在刷新范围后安排 200ms 防抖；快速连续选择只保留最后一个片段。输入编辑、手动播放、删除、切歌和关闭窗口继续取消 pending。该修复仍待 Windows EXE 人工复验。
...
- V2.4-C：Windows EXE 的 UI、OGG 手动试听与自动试听稳定性。
