...
- 测试基线：363 passed，12 skipped

本文记录当前稳定分支的完成度。历史需求书和 V2.3 实施计划保留其当时的设计上下文，不作为当前状态判断依据。
...
V2.4-A 音频试听位于独立分支 `feature/v2.4-audio-audition`，不包含在 `main` 中。该分支的基础能力是：对当前选中的完整片段使用源 `base.ogg` 手动试听，支持播放/暂停/恢复、循环开关、空格快捷键、有效倍速、播放头显示，以及切换片段时停止旧播放。
