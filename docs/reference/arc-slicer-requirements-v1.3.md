...
> * 将“非线性 Arc 中途截断”的行内风险提示列为 Gate 0 唯一剩余功能与验收项；
> * 明确有效单条 Arc 不会在语句内部折返；删除“反向 Arc”表述，统一使用“坐标递减的单向 Arc”；
...
* 非线性 Arc 截断的行内风险提示是 Gate 0 剩余必须项；完成并实测后 Gate 0 才正式关闭；
...
正常可读 AFF 的持续 Arc 应满足 `t1 < t2`。本项目只以正常时间正向推进的 Arc 为功能与测试对象；对不符合此条件的异常输入，仅要求解析器不崩溃且不误删无关事件，不推断其语义。

单条 Arc 在语句内部不会先朝一个方向移动、再折返。蛇形、回勾等视觉形状由多条 Arc 首尾连接形成。

需要正式验证的是**坐标递减的单向 Arc**：当 `x1 > x2` 和/或 `y1 > y2` 时，时间仍正向推进，坐标只是在该轴上从较大值单向移动到较小值；easing 名称仍表示同一时间缓动。不得因为坐标差为负而交换 `si` / `so` 或反转 x/y easing。
...
不存在 `ssi`、`sso`、`s si` 或 `s so`。坐标从大到小的单向移动时，不得因此交换 `si` 与 `so`；这不表示 Arc 在语句内部发生折返。
...
Gate 0 必须在时间段编辑行中，对会从中间截断非线性 Arc 的 `start` 或 `end` 边界显示一行低干扰提示；不得只写日志，也不得使用确认对话框或阻断导出。
...
提示仅在对应边界有风险时显示。Tooltip 应汇总该边界截断的 Arc 数量与 easing，例如：

```text
此切点截断 3 条非线性 Arc：si × 2，b × 1。
...
开始边界 `start_ms` 产生风险，当且仅当：

```text
low < start_ms < high
```

结束边界 `end_ms` 产生风险，当且仅当：
...
扫描必须覆盖根层 Arc、Timinggroup 内 Arc 与带 Arctap 的 Arc。带 Arctap 不改变风险判断。

单条 Arc 不存在“在语句内部先向一个方向再反向”的情况；本功能不分析相邻 Arc 的组合图形，只逐条判断它是否被当前切点裁去部分。

风险提示必须随歌曲、开始时间、结束时间、片段增删、加载配置与 songs 根目录切换而刷新。没有风险时提示隐藏，不保留空白占位，不影响保存、运行和输出结果。

当前阶段仍按现有端点插值与原 easing 保留规则导出；风险提示只用于就地说明。未来高精度拟合完成后，可在同一位置增加拟合方式选择或误差提示，但不得改变当前 Gate 0 的简单工作流。
...
8. 对坐标递减的单向 Arc 保持原 easing 语义，不因 x/y 坐标差为负而交换 `si` / `so`；
9. 对每段 Arc 坐标保持浮点字面量；
...
* 极短片段；
* 切点恰好落在物件时间上；
* 坐标递减的单向 Arc：`x1 > x2`、`y1 > y2` 与双轴均递减时，确认 easing 不被翻转；
* 单条 Arc 语句内不会出现先正向再反向的折返；蛇形与回勾应使用多条 Arc 首尾连接。不得为“单条 Arc 内部折返”设计功能或测试；
* 非线性 Arc 行内提示：start/end 分别标记、`s` Arc 不提示、贴合边界不提示、Tooltip easing 汇总与编辑时刷新；
