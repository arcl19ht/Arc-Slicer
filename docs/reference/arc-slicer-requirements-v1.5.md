...
> 历史规格说明：本文记录修订 1.5 时的需求基线与版本路线。文中的“尚未实现”、阶段状态和路线安排只反映当时状态，不应作为当前完成度判断依据。当前状态以 `README.md`、`AGENT.md` 和 `docs/status/current-development-status.md` 为准。

> 文档状态：V2.2 正式规格基线（修订 1.5，替代 1.4）
> 项目名称：Arc Slicer
> 当前阶段：Gate 0 已正式关闭；Version 2.1「导出元数据、练习曲包与双层输出工作流」已完成并作为当前稳定基线；Version 2.2「安全合并到外部目标壳 songs 根目录」进入正式待实现规格。
> 已锁定基线：
...
> - 建立 Version 2.2 外部目标壳安全合并的正式需求；
> - 明确 V2.2 不等于、不替换 `library_export`，且当前尚未实现；
...
# Version 2.2：安全合并至外部目标壳 songs 根目录

## 定位

V2.2 的目标不是简单复制文件，而是将：

```text
ArcSlicerData/out/current_export/songs/
```

以可预览、可校验、有备份、可回滚的方式，合并到用户选定的外部目标壳 `songs` 根目录。

V2.2 不等于、不替换 `library_export`：

* `library_export` 是工具自身长期累计导出包，位于 `ArcSlicerData/out/library_export/`；
* V2.2 面向用户指定的外部目标壳 `songs/` 目录；
* V2.2 只能以 `current_export/songs/` 作为本次输入，不应直接把任意 staging 或 library 内容写入外部壳；
* V2.2 当前为正式待实现需求，不属于已经完成能力。
...
不允许“生成后直接偷偷写入外部壳”。流程必须分为：

1. 按现有 V2.1 流程生成 `current_export`。
2. 用户选择外部目标壳的 `songs` 根目录，先检查合并计划，再明确确认执行合并。
...
* `current_export` 不存在或不完整；
* `current_export` 中没有 `songlist`；
* 目标 `songs` 目录不存在；
* 目标目录是工具自己的 `current_export` 或 `library_export`；
* 目标目录、受影响歌曲目录或 `pack` 目录是 Junction / symlink；
* 目标 `songlist` 无法解析；
* 目标 `packlist` 在需要时无法解析；
* 目标 `songlist` 内部存在重复 `song.id`；
* 输入 `songlist` 内部存在重复 `song.id`；
* 输入 `packlist` 内部存在重复 `pack.id`；
* 输入歌曲目录与 `songlist.id` 对不上；
* 输入 `songlist` 所引用的歌曲目录不存在；
* 输入 `song.set` 在目标或输入 `packlist` 中没有对应 `pack.id`；
* 目标歌曲目录存在、但 `songlist` 中没有对应 id；
* `songlist` 中存在 id、但目标歌曲目录不存在；
* 输入 pack 图片文件缺失；
* 同一 pack 图片文件名对应不同内容且会覆盖其他 pack 的资源。

其中“目标元数据与目录不一致”必须视为阻止项，不能猜测用户想覆盖还是修复。

## Songlist 合并规则

按 `song.id` 合并：

* 目标不存在该 id：新增 `songlist` 条目，并新增同名歌曲目录；
* 目标存在该 id，且目录结构一致：用本次 `current_export` 的完整歌曲条目替换目标条目，并替换同名歌曲目录；
* 目标存在该 id，但 metadata / directory 关系异常：阻止合并，要求用户先修复目标壳。

排序规则：

* 已有目标 `songlist` 条目保持原相对顺序；
* 更新条目保留原位置；
* 新增条目按 `current_export` 中的顺序追加到末尾。
...
* 目标不存在该 `pack.id`：追加 pack 条目；
* 目标存在该 `pack.id`：以 `current_export` 的 pack 条目替换目标条目，但保留目标 `packlist` 中其他无关条目。
...
* 文件内容相同：允许复用；
* 文件内容不同，且属于不同 `pack.id`：阻止合并；
* 文件内容不同，但属于同一 `pack.id` 的更新：允许覆盖，必须备份旧文件。

当输入 pack 更新既有 `pack.id`，且其 `img` 文件内容与目标同名文件不同：

1. 先检查目标 `packlist` 中是否还有其他 `pack.id` 引用该 `img` 文件名；
2. 若存在其他引用，阻止合并，要求该更新 pack 使用新的 `img_name`；
3. 若不存在其他引用，才允许覆盖该图片；
4. 实际覆盖阶段必须备份旧图片。

这条规则是为了避免更新 Pack A 时，意外改变同样引用该图片的 Pack B。

新 pack 使用新图片文件名：

* 直接复制，不影响其他 pack 图片。

更新 pack 改用新的 `img` 文件名：

* 复制新文件；
* 不自动删除旧图片，避免误删其他曲包正在引用的资源。

## 无 Packlist 输入时

当本次 `current_export` 未包含 `packlist` 时：

* 只允许合并到目标 `packlist` 中已经存在的 `song.set`。

若任一输入歌曲的 `set` 在目标 `packlist` 中不存在：

* 阻止合并；
* 不得仅追加 `songlist` 后留下无法进入的歌曲。

## 备份、写入与失败恢复

V2.2 必须采用“先校验、后备份、再写入”的事务式流程。

备份根目录：

```text
ArcSlicerData/backups/external_merge/<timestamp>/
```
...
V2.2 不做“图形化一键回滚界面”，但必须让日志显示备份路径。

## 共用导出前校验

同一次导出中，若两个片段生成完全相同的 `song.id`，必须在导出前阻止操作。
...
不要塞进 Songlist、Packlist 或导出目录卡片内部。
...
* 总导出包元数据读取不合法；
* 同一次导出中存在重复 `song.id`；
* 外部目标壳 `songs` 目录不存在或不是目录；
* 外部目标壳目录、受影响歌曲目录或 `pack` 目录是 Junction / symlink；
* 外部目标壳 `songlist` / `packlist` 解析失败；
* 外部目标壳 metadata 与目录不一致；
* 外部目标壳中存在重复 `song.id`；
* V2.2 合并计划存在阻止项；
* ffmpeg 不存在或失败。
...
8. V2.2 确认合并前不得写外部目标壳；
9. V2.2 写外部目标壳前必须完成受影响内容备份；
10. V2.2 写入失败时必须尽可能自动恢复。
...
* 目标元数据与目录不一致阻止；
* pack 图片同名同内容允许；
* pack 图片同名不同内容且不同 pack 阻止；
* 同 pack 更新图片允许且备份；
* 失败后自动恢复；
* 目标 Junction / symlink 阻止；
* 相同片段重复导出 ID 阻止。
...
* 目标壳实测中，一个新曲包至少要有一首歌列出 `ratingClass 0`、`1`、`2` 才能进入；
* 为保证单曲导出稳定性，工具对每条输出歌曲统一写入 `[0, 1, 2]` 占位结构；
* V2.2 必须以 `current_export/songs/` 作为外部壳合并输入；
* V2.2 不替换 `library_export`，也不把 `library_export` 直接写入外部壳；
...
* 自动裁切是否需要后续加入上 / 中 / 下焦点；
* V2.2 备份 manifest 的最终 JSON 字段形状；
* V2.2 UI 中冲突详情是否需要导出为文本报告；
